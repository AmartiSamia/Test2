...
pipeline {
  ...
  // 🚨 NEW: webhook + safe fallback polling (in case webhook fails)
  triggers {
    githubPush()                     // fires on GitHub webhook
    pollSCM('H/5 * * * *')           // every ~5 mins as a backup
  }
  ...
  stages {
      ...
      steps {
        ...
        script {
          ...
          if (params.DEPLOYMENT_ID?.trim()) {
            sh """curl -s -X POST "${BACKEND_BASE}/api/internal/stages" -H "Content-Type: application/json" -d '{"deployment_id":"${params.DEPLOYMENT_ID}","stage_name":"validate","status":"success"}' || true"""
          }
        }
      }
    }
    ...
    stage('Checkout') {
      steps {
        script { if (params.DEPLOYMENT_ID?.trim()) { sh """curl -s -X POST "${BACKEND_BASE}/api/internal/stages" -H "Content-Type: application/json" -d '{"deployment_id":"${params.DEPLOYMENT_ID}","stage_name":"checkout","status":"running"}' || true""" } }
    ...
    stage('Generate Dockerfile') {
      steps {
        script {
          if (params.DEPLOYMENT_ID?.trim()) { sh """curl -s -X POST "${BACKEND_BASE}/api/internal/stages" -H "Content-Type: application/json" -d '{"deployment_id":"${params.DEPLOYMENT_ID}","stage_name":"prepare","status":"running"}' || true""" }
...
CMD ["nginx","-g","daemon off;"]'''
          ...
          } else { echo '📄 Using existing Dockerfile' }
          sh 'echo "---- Dockerfile ----"; cat Dockerfile; echo "-------------------"'
          if (params.DEPLOYMENT_ID?.trim()) { sh """curl -s -X POST "${BACKEND_BASE}/api/internal/stages" -H "Content-Type: application/json" -d '{"deployment_id":"${params.DEPLOYMENT_ID}","stage_name":"prepare","status":"success"}' || true""" }
        }
      }
    }
    ...
    stage('Build Image') {
      steps {
        script { if (params.DEPLOYMENT_ID?.trim()) { sh """curl -s -X POST "${BACKEND_BASE}/api/internal/stages" -H "Content-Type: application/json" -d '{"deployment_id":"${params.DEPLOYMENT_ID}","stage_name":"build","status":"running"}' || true""" } }
        sh """
        ...
        """
        script { if (params.DEPLOYMENT_ID?.trim()) { sh """curl -s -X POST "${BACKEND_BASE}/api/internal/stages" -H "Content-Type: application/json" -d '{"deployment_id":"${params.DEPLOYMENT_ID}","stage_name":"build","status":"success"}' || true""" } }
      }
    }

    stage('Push to ACR') {
      steps {
        script { if (params.DEPLOYMENT_ID?.trim()) { sh """curl -s -X POST "${BACKEND_BASE}/api/internal/stages" -H "Content-Type: application/json" -d '{"deployment_id":"${params.DEPLOYMENT_ID}","stage_name":"push","status":"running"}' || true""" } }
        ...
        script { if (params.DEPLOYMENT_ID?.trim()) { sh """curl -s -X POST "${BACKEND_BASE}/api/internal/stages" -H "Content-Type: application/json" -d '{"deployment_id":"${params.DEPLOYMENT_ID}","stage_name":"push","status":"success"}' || true""" } }
      }
    }
...
${lbIpLine}""".stripIndent()
            ...
            timeout(time: 15, unit: 'MINUTES') { retry(2) { sh 'export PATH="$PWD:$PATH"; export KUBECONFIG="${KUBECONFIG_FILE}"; kubectl apply -f k8s.yaml' } }
            timeout(time: 10, unit: 'MINUTES') { retry(1) { sh 'export PATH="$PWD:$PATH"; export KUBECONFIG="${KUBECONFIG_FILE}"; kubectl rollout status deployment/${PROJECT_NAME} -n ${NAMESPACE} --timeout=300s' } }
          ...
          sh '''
            ...
            for i in {1..30}; do
              ...
              if [ -n "$EXTERNAL_IP" ] && [ "$EXTERNAL_IP" != "null" ]; then
                ...
                # Always try to update a project URL; if you also use deployments API, keep both:
  ...
  post {
    ...
    failure {
      ...
      script {
        ...
        // Optional: notify project on failure too
        sh """curl -s -X POST "${BACKEND_BASE}/api/internal/projects/${PROJECT_NAME}/status" -H "Content-Type: application/json" -d '{"status":"failed"}' || true"""
      }
